...
using AuditService.Tests.AuditService.WebApi.Extensions;
...
public partial class Assert
{
        ...
        List<PlayerChangesLogDomainModel> playerChangesLogMock, IDictionary<string, string> localizedKeys)
    {
        playerChangesLogResponse.ForEach(log =>
        {
            ...
            {
                Contains(nVal.Value, playerChangesLogMock.SelectMany(l => l.NewValues.Select(x => x.Value.Value)));
                Contains(nVal.Type, playerChangesLogMock.SelectMany(l => l.NewValues.Select(x => x.Value.Type)));

                var mockNewValues =
                    playerChangesLogMock.SelectMany(l => l.NewValues.Where(t => t.Value.Value == nVal.Value))
                        .FirstOrDefault();

                Equal(nVal.Label,
                    mockNewValues.Value.IsTranslatable ? localizedKeys[mockNewValues.Key] : mockNewValues.Key);
